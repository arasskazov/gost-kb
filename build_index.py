...
def load_metadata(standards_dir):
    """Load metadata from all standards."""
    metadata_list = []
    
    for std_dir in sorted(Path(standards_dir).iterdir()):
        ...
        # Add paths
        metadata['rel_path'] = str(std_dir.relative_to(standards_dir.parent))
        metadata['full_md_path'] = f"{std_dir.relative_to(standards_dir.parent)}/full.md"
        metadata['summary_md_path'] = f"{std_dir.relative_to(standards_dir.parent)}/summary.md"
        metadata['metadata_json_path'] = f"{std_dir.relative_to(standards_dir.parent)}/metadata.json"
...
standards/
├── 77-001/
│   ├── full.md           — полный текст стандарта (нормализованный)
