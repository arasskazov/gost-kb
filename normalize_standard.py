...
def normalize_content(raw_content, standard_num):
    ...
    # Fix section numbering - ensure proper markdown headings
    # Convert patterns like "1.  Область применения" at start of line to proper headings
...
def extract_title(raw_content):
    ...
    # Look for the main title after the header table
    lines = raw_content.split('\n')
    
    # Common patterns for titles
    title_patterns = [
        r'\*\*Система поддержки жизненного цикла изделия\*\*',
        r'\*\*(ИНФОРМАЦИОННАЯ МОДЕЛЬ ИЗДЕЛИЯ)\*\*',
        r'\*\*(Термины и определения)\*\*',
        r'\*\*(Общие требования)\*\*',
        r'\*\*(Модель жизненного цикла изделия)\*\*',
        r'\*\*(Основные положения)\*\*',
        r'\*\*(Виды программных средств поддержки жизненного цикла)\*\*',
        r'\*\*(Интероперабельность программных средств)\*\*',
        r'\*\*(Цифровые двойники изделий)\*\*',
        r'\*\*(Управление конфигурацией изделия)\*\*',
        r'\*\*(Управление данными об изделии)\*\*',
        r'\*\*(Классификатор видов деятельности)\*\*',
    ]
...
def extract_terms(raw_content):
    ...
    # Look for section 3 or 3.1 with terms
    # Pattern: "3.1.1 **термин**: определение"
...
def extract_keywords(raw_content):
    ...
    # Common keywords for these standards
    common_kw = [
...
def create_summary(raw_content, standard_num, metadata):
    ...
    # Extract key provisions (sample from main sections)
    key_provisions = []
...
"""
    ...
    full_md_path = os.path.join(dir_path, 'full.md')
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Usage: normalize.py <standard_number>")
        sys.exit(1)
    
    standard_num = sys.argv[1]
    raw_path = f"standards/77-{standard_num}/full.raw.md"
